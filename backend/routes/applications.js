...
router.post('/', [
    ...
    try {
        ...
        // Логируем действие
        await db.query(`
            INSERT INTO action_logs (action, details, ip_address)
            VALUES ($1, $2, $3)
        `, [
...
async function createUserFromApplication(application) {
    try {
        ...
        await db.query(`
            INSERT INTO play_limits (user_id, total_minutes, daily_limit_minutes, is_limited)
            VALUES ($1, 0, 600, true)
        `, [userId]);

        await db.query(`
            INSERT INTO trust_level_progress (
                user_id, current_level, email_verified, discord_verified, 
                minecraft_verified, reputation
            ) VALUES ($1, 0, false, false, false, 0)
        `, [userId]);
